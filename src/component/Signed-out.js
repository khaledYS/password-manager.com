    ...
    const SignIn = () => {
        return ( 
            <form data-name='SignIn' onSubmit={(e)=>{e.preventDefault() ; props.signUser(e.target.querySelector('input[type="email"]').value, e.target.querySelector('input[type="password"]').value)}}>
                    ...
                    <input required={true} type='email' spellCheck='false' placeholder='example@email.com' defaultValue='example@email.com'></input>
                    <input required={true} type='password' spellCheck='false' min={8} placeholder='Password' defaultValue='example@email.com'></input>
